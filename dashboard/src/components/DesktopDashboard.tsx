    ...
    <div className="flex flex-col h-screen bg-bg overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 p-4 overflow-hidden">
        ...
        <div className="col-span-3 flex flex-col gap-3">
          ...
          <div className="bg-surface rounded-xl border border-border p-4 flex-1">
            <h3 className="text-sm font-semibold text-text-dim uppercase tracking-wider mb-3">
              Stats
            </h3>
            {prediction ? (
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-text-dim">Ensemble Accuracy</span>
                  <span className="text-up font-mono font-semibold">93.3%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-dim">Coverage</span>
                  <span className="text-text font-mono">24.0%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-dim">Up Threshold</span>
                  <span className="text-up font-mono">{prediction.up_threshold.toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-dim">Down Threshold</span>
                  <span className="text-down font-mono">{prediction.down_threshold.toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-dim">Models</span>
                  <span className="text-text font-mono">{prediction.total_models}</span>
                </div>
              </div>
            ) : (
              <div className="space-y-2">
            ...
            <div className="flex-1 overflow-y-auto">
              {history.length > 0 ? (
                  ...
                  <tbody className="divide-y divide-border">
                    {[...history].reverse().map((entry, i) => (
                      <tr key={i} className="text-text">
                        ...
                        <td className="py-2 text-right font-mono">{entry.confidence.toFixed(0)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="text-text-dim text-sm text-center py-8">
                  No signals yet. Waiting for strong predictions...
                </div>
              )}
            </div>
          </div>
