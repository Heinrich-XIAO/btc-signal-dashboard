"""Retrain models using only data available at candle OPEN (no lookahead/cheating)."""
import pandas as pd
import numpy as np
import json
import joblib
...
# The problem: features are computed on completed candle T, but at T's open we only know T's open
# Fix: Shift all features by 1, so features[i] represent data available at the OPEN of candle i
# This means features[i] = completed data from candle i-1 and earlier
# The target stays the same: target[i] = (close[i+1] > close[i])

# Drop rows with NaN in target first
df = df.dropna(subset=["target"])

# Identify feature columns (everything except target and timestamp-related)
feature_cols = [c for c in df.columns if c != "target"]

# Shift all features by 1 (so row i uses data from candle i-1)
print(f"\nShifting {len(feature_cols)} features by 1 (using only prior completed candles)...")
for col in feature_cols:
    df[col] = df[col].shift(1)

# Drop rows with NaN after shifting
df_clean = df.dropna()
print(f"After shift: {len(df_clean)} rows")
...
print("(Features shifted by 1 — only prior completed candles used)")
...
metadata = {
    ...
    "note": "Features shifted by 1. Models trained on data available at candle OPEN (only prior completed candles).",
}
